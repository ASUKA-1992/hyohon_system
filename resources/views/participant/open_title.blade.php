...
<div class="text_center meeting_create">
    ...
    @if($participant->meeting_title_open == 0)
	    <p id ="main_text" class="font_size_20"></p>
	    <div>
	        <button id="show_text_button" class="min_button" onclick="open_text('{{ $participant->meeting->title }}')">テーマを見る!</button>
	    </div>
	@else
...
<script>
    ...
    function open_text(arg_text){
        $.ajax({
            type: "get",
            url: "{{ $participant->id }}/meeting_title_open",
            dataType: "json",
        })
        .done((res) => {
