...
ParticleEngine.prototype.createParticle = function()
{
	...
	//【追加】
	if(Array.isArray(this.positionBase)){
		selected_position_base = this.positionBase[Math.floor(Math.random() * this.positionBase.length)];
	} else {
		selected_position_base = this.positionBase;
	}

	if (this.positionStyle == Type.CUBE)
		particle.position = this.randomVector3( selected_position_base, this.positionSpread ); 
	if (this.positionStyle == Type.SPHERE)
	{
		...
		particle.position = new THREE.Vector3().addVectors( selected_position_base, vec3.multiplyScalar( this.positionRadius ) );
	}
	...
	{
		var direction = new THREE.Vector3().subVectors( particle.position, selected_position_base );
	...
	//【追加】色が配列か？
	color = this.selectedColor[Math.round(Math.random() * this.selectedColor.length)];
	if(color === undefined){
...
ParticleEngine.prototype.initialize = function()
{
	...
	for (var i = 0; i < this.particleCount; i++)
	{
		...
		if(this.front_bigger != 1){
			this.particleArray[i].velocity.z = 0;
		}
